...
const info = computed(() => {
  const personalityCode = getString('about_personality', 'INFJ-A');
  const personality = getPersonalityInfo(personalityCode);

  return {
    author: getBasic('author'),
    describe: getString('about_describe'),
    describeTips: getString('about_describe_tips'),
    photo: getString('author_photo'),
    exhibitionImg: getString('about_exhibition'),
    profile: getArray<{ label: string; value: string; color: string }>('about_profile'),
    personality: {
    ...
    motto: {
      main: getArray<{ value: string }>('about_motto_main'),
      sub: getString('about_motto_sub'),
    },
    socialize: getArray<{ name: string; url: string }>('about_socialize'),
    creation: getArray<{ name: string; url: string }>('about_creation'),
    versions: getArray<{ name: string; version: string }>('about_versions'),
    union: getArray<{ name: string; url: string }>('about_unions'),
    story: getString('about_story'),
  };
});

const runningDays = computed(() => calcRunningDays(getBasic('established', '2024-01-01')));
    ...
    <div id="two">
      ...
      <div class="information">
        ...
        <div class="about-layout Statistics">
          <span>{{ articlesTotal }}篇文章</span>
          <span>{{ categoriesTotal }}个分类</span>
          <span>{{ tagsTotal }}个标签</span>
