...
const footerMenus = computed(() => filterByKey('footer'));
const { friends, loading: isLoadingFriends, fetchFriends, getRandom } = await useFriendList();

const isExternalLink = (url: string) => {
  return url.startsWith('http://') || url.startsWith('https://');
};

const randomFriends = computed(() => getRandom(3));
