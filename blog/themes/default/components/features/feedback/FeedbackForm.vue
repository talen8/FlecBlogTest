...
const { submitFeedback, getFeedbackByTicketNo } = useFeedback();

const FEEDBACK_FILE_TYPES = [
  'image/jpeg',
  'image/jpg',
  'image/png',
  'image/gif',
...
const handleFileUpload = (event: Event, targetKey: keyof typeof fileArrayMap) => {
  ...
  for (const file of files) {
    const validationError = validateFile(file, FEEDBACK_FILE_TYPES);
    if (validationError) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const attachmentUrls = await uploadFiles(allFiles, '反馈投诉', FEEDBACK_FILE_TYPES);
        ...
        <div class="dialog-body">
          ...
          <div v-if="queryResult" class="query-result">
            ...
            <div class="result-item">
              <label>反馈时间：</label>
              <span>{{ formatDateTime(queryResult.feedback_time) }}</span>
            </div>
            ...
            <div v-if="queryResult.reply_time" class="result-item">
              <label>回复时间：</label>
              <span>{{ formatDateTime(queryResult.reply_time) }}</span>
            </div>
