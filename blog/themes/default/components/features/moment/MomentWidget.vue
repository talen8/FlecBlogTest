...
const { useWidgetSSR } = useMoments();
const { getNumber } = useTheme();

const momentsSize = computed(() => getNumber('moments_size', 30) || 30);

// 获取动态数据（支持 SSR）
const { data: moments } = await useWidgetSSR('moments-widget', momentsSize.value);
